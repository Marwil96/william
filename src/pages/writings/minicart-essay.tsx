...
const versions = [
  { label: "Silent", step: 0 },
  { label: "Admits the work", step: 1 },
  { label: "Commits first", step: 2 },
];

const Artifact = () => {
  const [active, setActive] = useState(0);
  return (
    <aside className="border border-dashed border-gray-400 bg-[#0d0d0d] mt-16 mb-10">
      <div className="px-3 py-2 border-b border-dashed border-gray-400 flex items-center justify-between">
        <span className="text-[10px] font-inter uppercase tracking-[0.2em] text-gray-500">
          Artifact
        </span>
        <span className="text-[10px] font-inter uppercase tracking-[0.2em] text-gray-500">
          The three versions
      ...
      >
        {versions.map((v, i) => (
          <button
            ...
            onClick={() => setActive(i)}
            className={clsx(
              "flex-1 py-3 text-[10px] font-inter uppercase tracking-[0.15em] text-center transition-colors border-r last:border-r-0 border-dashed border-gray-400 cursor-pointer",
              active === i
      ...
      <AnimatePresence mode="wait">
        <motion.div
          key={active}
          className="flex items-center justify-center p-6 md:p-10 min-h-[420px]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          <MockedCartProvider>
            <CartItems step={versions[active].step} />
          </MockedCartProvider>
        </motion.div>
      </AnimatePresence>
    </aside>
...
const MinicartEssay: NextPage = () => {
  return (
    <Layout
      title="A cart is not a list — William Martinsson"
      desc="Notes on what the design of a minicart is actually for."
      framerKey="writings-minicart-essay"
      project
    ...
    >
      <article className="flex flex-col mt-6 md:mt-12">
        ...
        {/* ─── Footer ─── */}
        <div className="border-t border-dashed border-gray-400 mt-6 pt-6 pb-12 flex items-center justify-between text-[11px] font-inter uppercase tracking-[0.15em] text-gray-500">
          <Link
